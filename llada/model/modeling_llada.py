...
class LLaDABlock(nn.Module):
    ...
    # NOTE: attention is causing problems with dual caching
    def attention(
        self,
        q: torch.Tensor,
        k: torch.Tensor,
...
class LLaDAModelLM(PreTrainedModel):
    """
    Extremely barebones HF model wrapper. Modified to enforce prefix caching at generation
    """
    ...
    def forward(
        ...
        #NOTE: changing from None to an actual loss computation
        loss = None
        ...
        return CausalLMOutputWithPast(
            loss=loss, 
            logits=logits,
            past_key_values=outputs.attn_key_values, # the graph is breaking when using dual caching
    ...
    @ torch.no_grad()
    def generate_with_prefix_cache(self, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        x = torch.full((prompt.shape[0], prompt.shape[1] + gen_length), mask_id, dtype=torch.long).to(self.model.device)
        ...
        for num_block in range(num_blocks):
            ...
            past_key_values = output.past_key_values # is this unique to LladaModelLM?
            ...
            while True:
                ...
                logits = self.model(x[:, current_block_start:], past_key_values=past_key_values, use_cache=True).logits
...
# NOTE: changing from AutoModel to AutoModelForCausalLM
#AutoModel.register(LLaDAConfig, LLaDAModelLM)
AutoModelForCausalLM.register(LLaDAConfig, LLaDAModel)