...
# trying to debug the graph break caused by dual caching
    ...
    while True:
        ...
        if args.use_cache:
            if args.if_cache_position:
                # NOTE: this breaks. for the time being, do not use prefix and suffix caching
                with torch._dynamo.config.patch(capture_dynamic_output_shape_ops=True):
                    compiled_generate_with_dual_cache = torch.compile(generate_with_dual_cache, backend="eager", fullgraph=True)
                    out, nfe = compiled_generate_with_dual_cache(model, prompt, steps=steps, gen_length=gen_length, block_length=args.block_size, temperature=0., remasking='low_confidence', threshold=args.threshold)
